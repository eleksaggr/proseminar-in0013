...
It is common to store long-lived data on the heap section of the memory, as the lifetime of the data is then not decided by the lifetime of its local function. Rust allows allocating data on the heap by using the \mintinline{rust}{Box<T>} type\cite{RustDoc-Box}. An example of this can be seen in figure \ref{box-intro}, where the \mintinline{rust}{Box::new()} method is called to construct a \mintinline{rust}{Box} object containing an integer, effectively placing it on the heap. A conceptual implementation of such a \mintinline{rust}{Box} is given in figure \ref{box-implementation}. 
\begin{figure}
\begin{rustcode}
fn main() {
    let b = Box::new(42);
...
\end{rustcode}
\vspace{-2em}
\caption{An exemplary implementation of the \mintinline{rust}{Box} type.}
