...
Usage of the \mintinline{rust}{unsafe} keyword allows to notify the compiler to be less strict in the block that was marked.
...
\begin{itemize}
        ...
        \item \mintinline{rust}{&T} and \mintinline{rust}{&mut T} follow LLVM's scoped noalias model, expect if the \mintinline{rust}{&T} contains an \mintinline{rust}{UnsafeCell<T>}. Unsafe code must not violate these aliasing guarantees.
        \item Mutating an immutable value/reference without \mintinline{rust}{UnsafeCell<T>}
        \item Invoking undefined behaviour via compiler intrinsics:
            \begin{itemize}
                        \item Indexing outside of the bounds of an object with \mintinline{rust}{std::ptr::offset} (offset intrinsic), with the exception of one byte past the end which is permitted.
                        \item Using \mintinline{rust}{std::ptr::copy_nonoverlapping_memory} (memcpy32/mempy64, intrinsics) on overlapping buffers.
            \end{itemize}
        \item Invalid values in primitive types, even in private fields/locals:
            \begin{itemize}
                        \item NULL/dangling references or boxes
                        \item A value other than \mintinline{rust}{false} (0) or \mintinline{rust}{true} (1) in a \mintinline{rust}{bool}
                        \item A discriminant in an \mintinline{rust}{enum} not included in its type definition
                        \item A value in a \mintinline{rust}{char} which is a surrogate or above \mintinline{rust}{char::MAX}
                        \item Non-UTF-8 byte sequence in a \mintinline{rust}{str}
            \end{itemize}
